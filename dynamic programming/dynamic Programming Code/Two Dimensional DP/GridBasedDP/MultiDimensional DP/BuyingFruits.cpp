...
Difficulty : Hard
EXPLANATION
This question can be solved using dynamic programming. Let's define dp[i][j] indicates the minimum cost of shopping if the jth fruit is purchased from shop i (considering only till ith shop). We can easily that there are three choices,

// If you are purchasing apples from ith shop, you can only purchase mangoes // or oranges from the previous shop. Choose the one which gives minimum cost.
